...
**Voice:** Riyo
**Status:** Witnessed end to end `20260808.040720` -- `gh` as `acme-owner` - Claude Code on pier (`claude.ai` - ping GREEN) - standing tmux session `pier` holds the long thought
...
The token is a secret and stays in the maintainer's hands alone -- spoken once to the pier in the next movement, held nowhere else, never written into any file this tree carries.
...
```
gh auth status
gh repo view acme-owner/grain --json name
git ls-remote https://github.com/acme-owner/grain.git HEAD
```
