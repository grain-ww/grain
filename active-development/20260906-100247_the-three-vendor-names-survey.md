...
**Asked by:** the maintainer, `20260906` -- *molt and archive any files, rules and documents associated with
...
until the maintainer circles it (`.claude/rules/molt.md`). This page is the map the seating is decided from.
...
different act from retiring a tool. **the maintainer's word decides whether gratitude retires with the
...
is the same document the maintainer has already named for the fleet-insight refresh. **The two jobs are one
...
direction question and it is the maintainer's.
