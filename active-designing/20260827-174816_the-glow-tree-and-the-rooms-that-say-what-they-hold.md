...
**Voice:** Kyri
**Status:** GRANTED `20260827` (`approve all doors`), mixed -- two measured surveys and a room-naming proposal -- the `src/` rooms, the `loops/` room, the `glow/gen/` letter fold, and FORA31 have all landed; the **deep debride** waits on its own precondition and on the maintainer's word (custody gate %5). What landed differs from what was proposed in two places, both recorded below.
...
unbuilt rung in exactly those words, and it waits on the maintainer. Everything else in both modules stands ready behind it.
...
4. **Then the socket**, on the maintainer's word, and not before: FORA31, where `constel/` meets
...
**The deep debride is not taken, and the reason is its own precondition.** the maintainer's word names it
