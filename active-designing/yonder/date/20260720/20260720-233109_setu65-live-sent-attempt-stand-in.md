...
*the maintainer approved Framework lean A (live SETU6.5 sent eyes). First pass had no device; Pixel then appeared on adb as charging+debug without `usb0`/`ncm`. Enabling NCM without `adb` dropped the debug interface; phone left the bus. Live **sent** still waits on a real NCM iface.*
