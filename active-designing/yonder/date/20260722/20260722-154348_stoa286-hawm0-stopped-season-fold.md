# STOA286 -- HAWM0 Stopped - Chapter Fold Check-In

*An Acme Corporation employee reading this gets the close after the maintainer stopped hawm0 cleanly: the host boot->harvest->HAWM1->session_root chain stays GREEN on disk, the AVD is down, and the Pixel remains the live USB device.*
...
adb after stop: Pixel `[redacted-serial]` only - no `emulator-*`.
