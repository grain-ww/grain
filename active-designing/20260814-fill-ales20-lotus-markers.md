...
Lindy-first, crux-first: markers are read on every editing session for years, and the crux -- keeping named positions **sorted and unique** so the nearest-marker query is exact -- is small and wholly tractable, composing ALES19's `move_to` and ALES14's `make` over their public APIs. It stays entirely within the read side (no module seam, no gate). A punch region tying playback back to the Mikrophone's capture remains a seam that waits for the maintainer's word.
