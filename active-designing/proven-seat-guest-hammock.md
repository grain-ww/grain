...
*the maintainer granted word for the standing recommendations (`20260712.071405` bench-clock). This hammock opens Genode/Sculpt G0 on the cloud QEMU path while Caravan stays the named horizon -- guest, never merger.*
...
## Enclosure (a laptop - this season)

| Layer | What holds |
|-------|------------|
| **Metal** | a laptop AMD Ryzen 7040 - Ubuntu 24.04 LTS - GNOME Wayland - native SVM - `/dev/kvm` on the host |
...
Signing: the maintainer's own keys on this host; lane key stays cloud; sandbox key stays ai-jail.

## Word

the maintainer: *"i grant my word for any recommendation."* Treated as the seat word for proven-seat metal (and the other parks that awaited word).
