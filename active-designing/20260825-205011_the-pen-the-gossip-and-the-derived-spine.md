...
**Voice:** Kyri
**Status:** Mixed -- design, proposal. **Move 1 is SEATED** on the maintainer's word `20260827.181605`: the law is `../.claude/rules/derived-spine.md`, the meter `../tools/r/reds_spine_derive_witness.rish`, seventeen control cases plus a replay against the collision that actually happened (`%292`, `20260827`). Moves 2 through 7 remain proposals, and their seats stay the maintainer's.
...
the index fold) and Move 1's recommendation answers the %230 gate -- **the seat stays the maintainer's**.
