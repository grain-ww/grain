...
The next Lotus rung can open the **timeline** that edits these reassembled samples (cut, gain, splice -- the DAW's first real gesture), or **clock** the stream to a real sample rate. The audio-interface **hardware** stays a paused research round, taken only on the maintainer's word.
