...
With a graft a keeper copies and moves a marked span as one. The next rung can offer the **choice of law** where a crossfade meets a grafted edit (a smooth join rather than a hard butt splice), a **named clipboard** so a span survives across masters, or -- a module seam, the maintainer's word -- carry a grafted span across the ALES0 audio **wire** as its own framed payload, tying an editable span to the Mikrophone's capture. The real two-channel sound-card write stays a paused hardware research round, taken only on the maintainer's word.
