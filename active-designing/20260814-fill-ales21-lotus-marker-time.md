...
With markers set and read in time, a keeper marks a session the way they hear it. The next rung can offer the **choice of law** where a crossfade meets a marked edit, name a **selection** (a marked span a keeper cuts or gains as one), or -- a module seam, the maintainer's word -- a **punch region** armed between two timed markers, tying playback back to the Mikrophone's capture. The real two-channel sound-card write stays a paused hardware research round, taken only on the maintainer's word.
