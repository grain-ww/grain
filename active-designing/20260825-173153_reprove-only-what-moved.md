...
**Voice:** Kyri
**Status:** Mixed -- design, proposal; the FAST/COLD ruling for Move 2 was GRANTED on the maintainer's word `20260825.181028`; the hit-rate instrumentation and lap-tail reorder remain its engineering gates
...
the maintainer asked two things. First: can the harness tell which proofs a `.rye` change actually
...
So the restart skip is first a policy amendment only the maintainer can make, and an engineering trick second. The design below makes the skip *safe to rule on*, and the ruling stays his.
...
**What it waits on, in order:** (1) the maintainer's ruling amending the FAST/COLD law for the cold open
...
hammock's own gate stands unchanged: an instrumented cost table first, and the maintainer's word.
...
standing for the maintainer's word.
