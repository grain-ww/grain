...
**Status:** Landed -- **A ran** on the maintainer's word `20260820.142246`, and **B ran** on his word `20260820.162747`; see the two addenda at the foot. The body below stands exactly as it was written, before either word came.
...
Whichever way it goes, the arc keeps climbing. This document changes nothing on its own; it hands the maintainer a number where a recollection stood.
...
the maintainer's word came, approving every standing recommendation including this one. **A landed.** Every check in the Caravan ladder is `pub` now, and a rung whose check is byte-for-byte the rung below's runs it there rather than keeping a second copy.
...
the maintainer's word came at the ladder-copy checkpoint, and it opened **B**. The
