...
**Status:** Vision -- Design — the tilak shapes for Kumara's public seat, stated with invariants before any implementation; zero chain calls, zero code, zero keys; **every wire name below is a candidate, held whole for the maintainer's word**
**Ground:** Twin Chapter Arc II, round r25; Arc III's residents round receives these shapes the day their names are worded
...
Seven rows, zero seated. The day the maintainer words them, Arc III's residents round lands each as one file in `til/` with one pedestal in `sur/` — and not an hour before.
...
*May the seam stay one sentence wide, the five shapes stay small enough to read aloud, the rival keep us honest at every row — and may the names arrive in the maintainer's voice, exactly when they are ready to be worn.*
