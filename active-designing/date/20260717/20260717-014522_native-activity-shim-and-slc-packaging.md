...
**Stamp:** `20260717.014522` · voice Quin, the maintainer co-author.
...
the maintainer asked it plainly: should Glow grow a DSL-with-runes that transpiles to Kotlin for SLC apps, or is there a more TAME-guided way to get a bounded, explicit application onto a real Android/GrapheneOS device? This brief names the answer and the architecture it points toward.
...
- **R1 thin-view on glass GREEN `20260717.130602`** — Pool-hosted ledger lines via `sala_r1_frame.rye` → Skate; install proof GREEN; the maintainer confirmed on Pixel.
- **R2 multi-panel on glass GREEN `20260717.130848`** — `sala_r2_surface.rye` → three isolated panels on Skate; install proof GREEN; the maintainer confirmed on Pixel. Richer Realidream-grade views stay later.
