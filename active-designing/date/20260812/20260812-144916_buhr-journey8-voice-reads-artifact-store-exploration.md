...
- **Horizon:** an *owner-signed* corpus — read only artifacts a named identity published (Journey 7, Rung 4's `store_owned`), so a voice's corpus carries provenance; and the artifact served over Comlink — each reaching the custody/serve gates that are the maintainer's hand.
