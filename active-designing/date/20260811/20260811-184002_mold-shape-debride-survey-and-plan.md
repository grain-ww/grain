...
1. **Loaded Glow context** — the shape-vs-study judgment is per-symbol (an identifier that names Grain's concept renames; one that names the Hoon parallel a study twin reads stays), and it is a language-design call in the maintainer's own language, not a mechanical substitution.
...
## Resolution `20260811.184434` — leave the residue (the maintainer's word)

the maintainer's word: **leave the residue.** The debride closes here. The primary rename already landed at `20260720`; what remains is the record and the teaching, not drift:
