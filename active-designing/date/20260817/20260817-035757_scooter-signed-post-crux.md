...
Signing with the maintainer's own Kumara key is the custody gate; this rung signs only demo keeper seeds. The Comlink transport that carries a signed post between piers reaches the serve gate. Both stay the maintainer's hand -- the library is what the agent builds and proves.
