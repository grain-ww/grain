...
**Loom is the *proposed* name, and it must not be seated until a whole-tree grep and the maintainer's word clear it.** The comlink-tendency asks for a clear, warm, safe word that collides with nothing already loved — and here the grep tells an honest story that a reader deserves up front:
...
| **Loom** | the weaving image, jidoka lineage | **encumbered** — collides with the reds-first metaphor (3 hits); adopt only if the maintainer decides the watcher sense and the promoted-guard sense can share the word |
...
- **The name waits for the grep and the word.** "Loom" is proposed and encumbered; nothing seats until a whole-tree grep and the maintainer's decision. Custody first applies to names as much as to keys: this brief builds nothing and claims no clearance.
