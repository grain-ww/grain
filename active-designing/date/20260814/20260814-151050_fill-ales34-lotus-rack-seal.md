...
With a sealed slot sheet a keeper's saved rack is caught the moment it is corrupt rather than silently mis-parsed. The next rung can **sign** the seal with a Kumara key — proving *who* wrote a rack, not only that it arrived whole — a module seam taken on the maintainer's word; or carry a slot across the ALES0 audio **wire** as its own framed payload, also a seam. The real two-channel sound-card write stays a paused hardware research round, taken only on the maintainer's word.
