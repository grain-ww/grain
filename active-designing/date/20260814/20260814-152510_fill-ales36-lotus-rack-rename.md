...
**Chapter:** the Six-Chapter double-seat, Chapter C thread (Lotus · the creative suite) · waymark **ALES** · rung **ALES36**
...
With rename in place, the purely-local rack primitives are complete (file, paste, drop, save, seal, load, merge, rename). The next durable purely-local rung is a **session file** — one sealed sheet carrying more than one rack, so a keeper saves a whole project rather than one rack. The **signed** carry (who wrote a rack, via a Kumara key) and the **wire** carry stay module seams for the maintainer's word; the audio-interface hardware stays a paused research round.
