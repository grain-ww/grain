...
With a saveable slot sheet a keeper's whole rack persists across a session close and travels between benches as plain text. The next rung can give the sheet a **content digest** (a Sha256 line over the record, so a rack sheet verifies before it is trusted, mirroring the ALES0 wire's deframe), or — a module seam, taken on the maintainer's word — carry a slot across the ALES0 audio **wire** as a frame so a filed span travels live between two Lotus benches. The real two-channel sound-card write stays a paused hardware research round, taken only on the maintainer's word.
