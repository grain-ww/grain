...
**Chapter:** the Six-Chapter double-seat, Chapter C thread (Lotus · the creative suite) · waymark **ALES** · rung **ALES38**
...
With the seal lifted to one place, the **session file** rung is clean composition — `begin`, render the rack sheet and the cue sheet into one body over one master, `finish`, and a single `open` verifies the whole project before any of it is trusted. The **signed** carry (who wrote a sheet, via a Kumara key) and the **wire** carry stay module seams for the maintainer's word; the audio-interface hardware stays a paused research round.
