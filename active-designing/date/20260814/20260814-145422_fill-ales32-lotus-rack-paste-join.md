...
With a joined rack paste a keeper drops any filed span into the middle of a mix by name without a click. The next rung can let a rack **travel as text** (a saveable slot sheet, mirroring ALES22's cue sheet, so the whole rack persists across a session close), or — a module seam, the maintainer's word — carry a slot's held span across the ALES0 audio **wire** as a frame so a filed span travels between two Lotus benches. The real two-channel sound-card write stays a paused hardware research round, taken only on the maintainer's word.
