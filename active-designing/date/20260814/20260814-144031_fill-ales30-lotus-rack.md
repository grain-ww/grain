...
With a rack a keeper carries a handful of named spans through a whole session and pastes each by name. The next rung can let a rack **travel as text** — a saveable slot sheet, mirroring ALES22's cue sheet, so the rack persists across a session close — let a pasted slot **join rather than butt** its neighbour (ALES26's crossfade join at the paste seam), or — a module seam, the maintainer's word — carry a slot's held span across the ALES0 audio **wire** as a frame so a filed span travels between two Lotus benches. The real two-channel sound-card write stays a paused hardware research round, taken only on the maintainer's word.
