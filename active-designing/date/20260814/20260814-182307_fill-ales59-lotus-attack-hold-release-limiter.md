...
**Waymark:** ALES · **Rung:** ALES59 · **Chapter:** the Six-Chapter double-seat, Chapter C thread (Lotus · the creative suite)
...
Software only, purely local. Bounded in-process i16 PCM buffers, siloed to `lotus/`. No socket, network, keys, funds, device, or sample rate; attack, release, and hold counted in sample **indices**. **No lookahead** — a slow attack lets a fast transient briefly overshoot the ceiling before the envelope catches it (this is an honest gain-reducer over time, not a sample-clamp); a true brickwall wants a lookahead delay line, taken only on the maintainer's word.

## What it opens

With the gate, compressor, and limiter all holding, the **held dynamics trio is whole** — the mirror of the un-held trio, over one proven held follower. The lookahead delay line for a true brickwall stays the the maintainer's-word horizon.
</content>
