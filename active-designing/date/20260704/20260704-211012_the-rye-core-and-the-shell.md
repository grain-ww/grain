...
**Status:** Proposed — pattern affirmed on recent metal; graduation to spec on the maintainer's word
...
**Lap three — module home.** Core moves from `work-in-progress/` or `tools/` into its module directory when the second consumer or the maintainer's word opens the gate.
