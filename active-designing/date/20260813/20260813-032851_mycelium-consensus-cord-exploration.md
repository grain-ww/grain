...
design read (over TigerBeetle + Mysticeti)," first among the Chapter-D rungs the harness itself named.
...
(the maintainer's hand). Composes over public API only — `mycelium/fold`'s minting and folding, `tally/kumara`'s
