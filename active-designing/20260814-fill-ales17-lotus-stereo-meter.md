...
With play, loop, and meter all stereo, the read side is genuinely two-channel end to end. The next rung can name a **scrub** window (a small movable read a keeper drags), meter a **looping** stereo region live (ALES16's wrap under the ALES17 meter), a **punch region** tying playback back to the Mikrophone's capture (a module seam, the maintainer's word), or offer the keeper the **choice of law** on either axis. The real two-channel sound-card write a stereo master ultimately feeds stays a paused hardware research round, taken only on the maintainer's word.
