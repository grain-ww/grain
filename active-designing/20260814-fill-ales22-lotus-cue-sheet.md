...
With a cue sheet a keeper's marks outlive the session and travel to another hand. The next rung can name a **selection** (a marked span a keeper cuts or gains as one), offer the **choice of law** where a crossfade meets a marked edit, or -- a module seam, the maintainer's word -- carry the cue sheet across the ALES0 audio **wire** as its own framed payload, tying a shared marker map to the Mikrophone's capture. The real two-channel sound-card write stays a paused hardware research round, taken only on the maintainer's word.
