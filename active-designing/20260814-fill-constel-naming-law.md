...
A **Constel constellation** (a whole fake network) is a hyphen-joined run of `1 <= n <= max_ships` (8) valid ships, within `max_constellation_bytes` (64) total. The `xx` - `xz` - `xn` - `xw` digraphs are explicitly welcome -- they read as unmistakably fake and match the self-invented `acme-owner` / `acme-owner` family.
