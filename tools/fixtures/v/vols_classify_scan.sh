#!/bin/sh
# tools/fixtures/v/vols_classify_scan.sh -- VOLS survey census, classified by kind and guard.
#
#   sh tools/fixtures/v/vols_classify_scan.sh <root> <guardlist>
#
...
#                    identifier the labeling law deliberately keeps (the maintainer's prose-only
...
census=$(sh tools/fixtures/v/vols_survey_scan.sh "$root" 2>/dev/null | grep '^site=' || true)
