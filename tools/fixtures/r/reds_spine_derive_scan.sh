...
# seated 20260827 on the maintainer's word.
#
  ...
  while read -r kind n up_stamp a b; do
    case "$kind" in
      published_double)
        ...
        # meet head on. That deadlock is the maintainer's word rather than a lap's, and it stands booked at
        ...
        detail "detail: published_double %$n -- the anointed spine binds this number to BOTH $up_stamp and $a; rule 3 holds for each, so no lap may move either. the maintainer's word, booked 20260907.014654"
        ;;
      rebinding)
...
# rule 3 holds for both rows, so neither number may move, and the deadlock is the maintainer's word (booked
