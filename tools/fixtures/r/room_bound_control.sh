#!/bin/sh
# tools/fixtures/r/room_bound_control.sh -- a room built to break the bound, so the gate is proven.
#
# WHY. tools/fixtures/r/room_bound_scan.sh refuses a room over its bound. A gate that has never
...
cd "$work"
over_out="$(sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 2>&1 || true)"
over_code=0
sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 >/dev/null 2>&1 || over_code=$?
...
empty="$(mktemp -d)"
cd "$empty"
missing_out="$(sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 2>&1 || true)"
missing_code=0
sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 >/dev/null 2>&1 || missing_code=$?
cd /
rm -rf "$empty"
...
sweep_out="$(sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 2>&1 || true)"
sweep_code=0
sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 >/dev/null 2>&1 || sweep_code=$?
# The shelf bound proven from both sides (seated 20260828, the maintainer's word): the same 3-file day
...
shelfgate_out="$(ROOM_SHELF_BOUND=2 sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 2>&1 || true)"
shelfgate_code=0
ROOM_SHELF_BOUND=2 sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 >/dev/null 2>&1 || shelfgate_code=$?
shelfat_code=0
ROOM_SHELF_BOUND=3 sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 >/dev/null 2>&1 || shelfat_code=$?

# The other side. Remove both plants and both readings must fall silent.
rm -rf alpha beta
git add -A >/dev/null 2>&1
bare_out="$(sh "$root/tools/fixtures/r/room_bound_scan.sh" 2 2>&1 || true)"
cd /
rm -rf "$pen"
