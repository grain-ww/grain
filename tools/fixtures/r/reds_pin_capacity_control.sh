...
# and raising a bound is the maintainer's word -- so it is reported and never gated. This is the scan
...
# row naming the maintainer counts, a row naming a custody gate counts, an ordinary open row does not, and
# a CLOSED row naming the maintainer counts for nothing at all, since a closed row holds nobody up.
...
{ echo '# REDS -- a pen ledger'; echo
  held_row 1 open "the repair wants the maintainer's word"
  held_row 2 open "held behind custody gate %1"
  held_row 3 open "the sweep is a lap's own"
  held_row 4 closed "this one wanted the maintainer's word once"
} > "$pen/j/REDS.md"
...
say "the the maintainer row is named"                 "$(has "$out" 'detail: pin_held %1')"
...
say "a the maintainer row is never named unheld"      "$([ "$(has "$out" 'detail: pin_unheld_row %1')" = no ] && echo yes || echo no)"
...
say "and still names the three held doors"    "$(has "$out" "each is the maintainer's word")"
out=$(run_scan "$pen/a" REDS_PIN_BOUND=100000)
