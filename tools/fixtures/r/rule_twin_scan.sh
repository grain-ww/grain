...
#                       the law, and that is the maintainer's word (below). A gate here would refuse the
...
#
# Whether either rule is MIRRORED stays the maintainer's word -- a `.claude/rules/*.md` page loads on every
...
# safety rule. That decision is the maintainer's word; this meter's job is to keep the count honest until
# it is made.
#
# USAGE
