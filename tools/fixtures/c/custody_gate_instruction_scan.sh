#!/bin/sh
# tools/fixtures/c/custody_gate_instruction_scan.sh -- an instruction file may name a custody-gated
...
# the maintainer holds by hand. That file was repaired by hand, and the SAME sentence stood on in
...
# gate %1 holds every refresh at the maintainers own hand`. Prose ABOUT force-pushing that names no command
...
#
# USAGE
#   sh tools/fixtures/c/custody_gate_instruction_scan.sh                # the real loop-prompt surface
#   sh tools/fixtures/c/custody_gate_instruction_scan.sh report <file>  # one planted file, for the pen
#
...
cat > "$work/read.awk" <<'AWK'
{
  ...
  for (i = 1; i <= n; i++) {
    ...
    lc = tolower(c)
    refused = (lc ~ /never/ || lc ~ /refus/ || lc ~ /not yours/ || lc ~ /custody/ || lc ~ /the-maintainer/ || lc ~ /manual gate/ || lc ~ /crossing a gate/ || lc ~ /stop/ || lc ~ /his hand/ || lc ~ /gate %/)
