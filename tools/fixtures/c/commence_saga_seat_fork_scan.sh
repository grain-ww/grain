...
#
#   sh tools/fixtures/c/commence_saga_seat_fork_scan.sh
#
# Law: no duty reports a total until its planted control reads correctly.
# Seating: the maintainer's word approve - fork waits - return_surface_p59 not consumed.
set -eu
...
# --- prose saga seated (the maintainer approve) ---
...
rg -q "the maintainer's word \*approve\*" "$SAGA_PROSE" || {
  echo "seat_saga=failed"
  echo "verdict=misread"
...
rg -q 'Chapter Close Narratives' "$SAGA_README" || {
  echo "seat_saga=failed"
  echo "verdict=misread"
...
rg -q 'remains available if the maintainer names' "$GEODE" || {
  echo "seat_fork=failed"
  echo "verdict=misread"
...
echo "seat_story=M8_witness+M8_narrative>M9_ascent_measured>saga_SEATED>fork_awaits_the maintainer"
