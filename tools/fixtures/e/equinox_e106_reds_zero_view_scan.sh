...
#
#   sh tools/fixtures/e/equinox_e106_reds_zero_view_scan.sh
#
...
sh tools/fixtures/r/reds_row_present.sh 33 >/dev/null || {
  echo "reds_row=failed"
  echo "verdict=misread"
  echo "detail=want_row_33"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh 'A zero in a report is a claim about the instrument' >/dev/null || {
  echo "reds_row=failed"
  echo "verdict=misread"
  echo "detail=want_zero_view_taught_law"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh 'Look where the thing would be before calling it gone' >/dev/null || {
  echo "reds_row=failed"
  echo "verdict=misread"
...
# A FLOOR, NEVER AN EQUALITY (`20260909.001500`, the maintainer's word). This read `rows=33` exactly, which
...
LEDGER=$(sh tools/fixtures/r/reds_ledger_scan.sh)
echo "$LEDGER"
