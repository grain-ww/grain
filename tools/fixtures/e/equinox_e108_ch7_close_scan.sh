...
#
#   sh tools/fixtures/e/equinox_e108_ch7_close_scan.sh
#
...
FASCIA_SH=tools/fixtures/f/fascia_metric_v0.sh
REDS=construction/REDS.md
ZERO=tools/fixtures/z/zero_view_scan.sh
M3=tools/fixtures/o/oldness_census_scan.sh
M4=tools/fixtures/r/radiant_h1_fence_scan.sh
...
# Shred: e108 named opens-ch8; e109 refined to the-maintainer-gated mode (duty not seat)
SHRED_MODE=unknown
if rg -q 'the maintainer-gated' "$MAP" && rg -q 'duty is not a seat|A duty is not a seat' "$MAP"; then
  SHRED_MODE=the-maintainer_gated_mode
...
echo "seat_map_shred_gate=the-maintainer_word"
...
for n in 34 35 36 37; do
  sh tools/fixtures/r/reds_spine_grep.sh "^\\| ${n} \\|" >/dev/null || {
    echo "reds_cross=failed"
    echo "verdict=misread"
...
sh tools/fixtures/r/reds_spine_grep.sh 'git ls-files' >/dev/null || {
  echo "reds_cross=failed"
  echo "verdict=misread"
  echo "detail=want_row_34_git_ls_files"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh 'Verify a zero' >/dev/null || {
  echo "reds_cross=failed"
  echo "verdict=misread"
  echo "detail=want_row_35_verify_zero"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh 'Fence-aware' >/dev/null || {
  echo "reds_cross=failed"
  echo "verdict=misread"
  echo "detail=want_row_36_fence_aware"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh 'No backtick' >/dev/null || {
  echo "reds_cross=failed"
  echo "verdict=misread"
...
# A FLOOR ON AN ADVANCING REVISION (`20260909.001500`, the maintainer's word). This read `metric_rev=i9`
