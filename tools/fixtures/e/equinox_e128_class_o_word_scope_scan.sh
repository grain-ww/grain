...
#
#   sh tools/fixtures/e/equinox_e128_class_o_word_scope_scan.sh
#   sh tools/fixtures/e/equinox_e128_class_o_word_scope_scan.sh prove-red
#
# Law: Class O membership is a measured property; the census is authority.
# Law: the maintainer's word is on the class and/or rooms -- not on each filename.
# Law: kg with approvals seats the framing; it does not open the cut.
set -eu

MODE=${1:-}
CONTROL_SCAN=tools/fixtures/c/census_control_scan.sh
SHED=tools/fixtures/s/shed_census_scan.sh
...
ELDER=tools/fixtures/e/equinox_e127_on_touch_scan.sh
