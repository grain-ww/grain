...
#
#   sh tools/fixtures/e/equinox_e135_mantrapod_affordable_scan.sh
#   sh tools/fixtures/e/equinox_e135_mantrapod_affordable_scan.sh prove-red
#
...
E112=tools/fixtures/e/equinox_e112_date_dialect_witness_scan.sh
E115=tools/fixtures/e/equinox_e115_instrument_suite_scan.sh
...
# Elder pitch must remain dated testimony (Reya - the maintainer - Rye OS) -- not rewritten
...
rg -q 'the maintainer' "$ELDER_PITCH" || {
  echo "elder_pitch=failed"
  echo "detail=want_elder_the-maintainer"
  echo "verdict=misread"
  exit 1
}
...
# REDS 46
sh tools/fixtures/r/reds_row_present.sh 46 >/dev/null || {
  echo "reds=failed"
  echo "detail=want_row_46"
  echo "verdict=misread"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'afford to run|stops being run' >/dev/null || {
  echo "reds=failed"
