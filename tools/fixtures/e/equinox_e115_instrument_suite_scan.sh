...
#
#   sh tools/fixtures/e/equinox_e115_instrument_suite_scan.sh
#
# Law: seat the suite; do not manufacture meters.
# Remaining after this seat: the maintainer-gated (fork - breach - shred - names).
set -eu

CONTROL_SCAN=tools/fixtures/c/census_control_scan.sh
SUITE=tools/fixtures/i/instrument_suite_scan.sh
ALMANAC=rye-learning-process/GLOW_ALMANAC.md
PRIN=tools/gen/chapter/prin_scope.rish
MAP=construction/EQUINOX_SEAT_MAP.md
REDS=construction/REDS.md
ELDER=tools/equinox/witness/equinox_e114_thing_not_name_witness.rish
...
for p in \
  "$SUITE" \
  tools/gen/chapter/instrument_suite_witness.rish \
  tools/fixtures/s/sundial.sh \
  tools/fixtures/f/fascia_metric_v0.sh \
  tools/gen/chapter/census_control_witness.rish \
  tools/gen/chapter/shed_census_witness.rish \
  tools/equinox/witness/equinox_e111_date_dialect_witness.rish \
  tools/gen/chapter/oldness_census_witness.rish \
  tools/gen/chapter/radiant_h1_fence_witness.rish \
  tools/fixtures/r/radiant_lint_scan.sh \
  tools/fixtures/e/equinox_e113_fascia_health_scan.sh \
  tools/gen/chapter/thing_not_name_witness.rish
do
...
sh tools/fixtures/r/reds_row_present.sh 39 >/dev/null || {
  echo "reds_keep=failed"
  echo "verdict=misread"
  exit 1
}
MONO=$(sh tools/fixtures/r/reds_ledger_monotone_scan.sh)
echo "$MONO"
...
echo "remaining=the-maintainer_gated"
...
echo "story=instrument_suite_10>counsel_nine_plus_thing_not_name>the-maintainer_gated_remainder>128_reserved>fork_waiting"
