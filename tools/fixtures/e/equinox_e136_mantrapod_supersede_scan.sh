...
#
#   sh tools/fixtures/e/equinox_e136_mantrapod_supersede_scan.sh
#   sh tools/fixtures/e/equinox_e136_mantrapod_supersede_scan.sh prove-red
#
...
DATED_GUARD=tools/fixtures/d/dated_guard_scan.sh
...
E135=tools/fixtures/e/equinox_e135_mantrapod_affordable_scan.sh
...
rg -q 'the maintainer' "$ELDER" || {
  echo "elder=failed"
  echo "detail=want_the-maintainer"
  echo "verdict=misread"
  exit 1
}
rg -q 'Rye OS' "$ELDER" || {
...
rg -q '^\*\*Voice:\*\* Riyo|the maintainer|runs Grain on' "$ELDER" && {
  echo "elder=failed"
...
# sweep of 20260823.101837 rewrote `the maintainer the maintainer` to `the maintainer` across 35 references
...
rg -q 'the maintainer|the maintainer the maintainer' "$NEW" || {
  echo "new_pitch=failed"
...
echo "new_pitch_living=riyo_the-maintainer_grain"
...
sh tools/fixtures/r/reds_row_present.sh 47 >/dev/null || {
  echo "reds=failed"
  echo "detail=want_row_47"
  echo "verdict=misread"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'dated|immutable|supersede' >/dev/null || {
  echo "reds=failed"
