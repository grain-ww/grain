...
#
#   sh tools/fixtures/e/equinox_e142_q2_cli_doc_scan.sh
#   sh tools/fixtures/e/equinox_e142_q2_cli_doc_scan.sh prove-red
set -eu
...
# R51
sh tools/fixtures/r/reds_row_present.sh 51 >/dev/null || {
  echo "reds=failed"
  echo "detail=want_row_51"
  echo "verdict=misread"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'leans on a name|Crossing Chapter|working lean' >/dev/null "$COUNSEL" "$DUTY" || {
  echo "reds=failed"
...
rg -qi 'seat is the maintainer|seat the maintainer|the maintainer.s' "$DUTY" "$COUNSEL" || {
  echo "lean=failed"
  echo "detail=want_seat_the-maintainers"
  echo "verdict=misread"
  exit 1
}
echo "lean=honored"
...
AMPH_N=$(git ls-files -s 'amphora/*' | awk '$1 != "120000"' | wc -l | tr -d ' ')
...
test "$AMPH_N" -ge 8 || {
  echo "census=failed"
  echo "detail=want_amphora_at_least_8_got_$AMPH_N"
  echo "verdict=misread"
  exit 1
}
