#!/bin/sh
# tools/fixtures/i/index_fold_scan.sh -- an index folds with the room it describes.
#
...
# while days sit flat, and folding the room is the maintainer's word (.claude/rules/session-logs.md).
#
# USAGE
#   sh tools/fixtures/i/index_fold_scan.sh          # census -- key=value lines
#   sh tools/fixtures/i/index_fold_scan.sh list     # every stale row, one per line
#
