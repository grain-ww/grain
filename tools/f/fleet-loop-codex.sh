...
# the maintainer's word, with two clauses he named: **bare**, and **loose**.
#
# BARE BY CONSTRUCTION. This pier runs without jails (the maintainer's word `20260906`), so there is no
...
# acceptable here because this pier runs bare by the maintainer's word, so the enclosure question is
...
# Fleet default selected by the maintainer on 20260912. Explicit CODEX_MODEL overrides remain supported.
