...
#
# Earth ships (names seated 20260904 on the maintainer's word): incense, pheromone,
...
#
# ANCHORED TO ITS OWN TREE (the maintainer's word 20260829: a loop sources from its own folder
...
# the maintainer's word lived here and not there. All six are lookups into construction/fleet-roster.kyri
...
while [ "$(date +%s)" -lt "$deadline" ]; do
  ...
  # CLOCKOUT, seated `20260909` on the maintainer's word, molted from `drain`: a shift worker finishes the
