...
# the maintainer's. `?&` and `?|` are the ancestor's own spellings, seated as peer names by
...
# arrives WHEREVER it arrives, and the day the maintainer names these two the list empties and the gate
...
for glyph in $unnamed_glyphs; do
  case " $UNNAMED_EXEMPT " in
    ...
    *)
      ...
      echo "detail=exempt is '$UNNAMED_EXEMPT'; a rune is earned by a law, and a name by the maintainer's word"
      exit 1
      ;;
  esac
done
...
# `foundations/20260830-011530_a-rune-is-earned-by-a-law.md`, and naming them stays the maintainer's word.
