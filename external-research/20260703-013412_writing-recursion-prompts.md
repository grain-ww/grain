...
**Three -- every branch is pre-decided.** The best prompts read like a switch statement over the maintainer's future words: Door 3 *for* means these exact commits at these exact numbers; Door 3 *against* means those. An agent should never stand at a fork the maintainer did not already resolve on paper. Where a genuine fork is possible and unresolved, the prompt says *stop-and-park* -- the fork itself is the deliverable.
...
**Seven -- every round ends in a summary with a Questions section.** The summary is the agent's account in the house voice; the Questions section is the standing interface back to the maintainer and Claude. A round without questions is a round that decided things silently.
