...
- **The session-log rule sync.** Matched to the profile of record: Claude Code, `claude-opus-4-8`, the a cloud provider VPS pier with the a tablet as the hand device, and the `.kyri` filename default (the notation molted from Bron). This session's own four mis-attributed logs were corrected to the truth -- a factual model-field fix the rule allows.
...
- **The eight-season itinerary.** The six-season expansion molted to eight, folding in Chapter G (open media primitives -- color, fonts, video) and Chapter H (open-weight intelligence for the inference hats). The color module that Chapter G names as its crux is already built and GREEN.
