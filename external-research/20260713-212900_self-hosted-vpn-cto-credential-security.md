...
- **No third party sees the traffic's metadata, because there is no third party.** A commercial VPN is itself a man-in-the-middle you have chosen to trust; a self-hosted one removes that trust requirement entirely -- the only party who can see decrypted traffic is the maintainer's own infrastructure.
...
- **The server**: a small VPS (1 vCPU, 1 GB RAM is plenty for WireGuard itself) from a provider the maintainer controls billing and root access for -- Hetzner, DigitalOcean, a cloud provider, or self-owned hardware on a static IP all work; the requirement is *root access and full disk control*, not a specific brand.
