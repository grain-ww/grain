...
*Named by the maintainer (`2026-07-02`). Puddle is a small word for many bounded worlds -- each whole, each shallow from the host's view, each deep to the agent inside.*
...
the maintainer asked for the sources themselves, so this pass reads them directly and brings back what they teach.
