...
**Language:** EN
**Stamp:** 20260712.093812 (the maintainer's clock, verbatim)
**Style:** Gauge (see `../context/GAUGE_STYLE.md`)
**Status:** Proposed ordering -- research for understanding / counsel. Rung sequence is a recommendation; the maintainer words which open.
...
These need only a session and, where marked, the maintainer's word. They are the ground everything else stands on.
...
- **A5 - Seat the parked names** on the maintainer's word: Flax/Bark/Oat/Walnut (Brushstroke palette); release Loam and Ember from park or keep parked; the three verdict RTAC rows (Kumara rotation, Comlink fade-from-birth, genode seams). **Blocker: the maintainer's word only.**
...
- **B1 - G0-complete on metal** -- interactive Sculpt / Leitzentrale under host KVM, witnessed by serial banner plus the maintainer's eyes on the glass. **Blocker: A3 (the KVM lane) -- or the existing outside-jail path for one more lap.**
...
- **C2 - Kumara rotation facts** -- old-key-signs-new, monotonic counter, PUBKEYS prose history made checkable law. **Blocker: none technical; wants the maintainer's word (touches keys).**
...
**A1 -> A2 -> A3 -> B1 -> B2** is the spine: clear the parity debt, file and seat the enclosure so KVM is honest, boot the seat to completion, prove the smallest guest. Everything in C and D hangs off that spine plus its own named module rung. A4, A5, and C2 run beside the spine on the maintainer's word without blocking it.
