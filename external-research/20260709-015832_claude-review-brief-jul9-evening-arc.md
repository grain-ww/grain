...
**Register:** Decision briefing -- what landed after the iOS session handoff, what counsel answered, where the bench sits, what the maintainer and Claude should open next
...
| Sketchbook shelf-only | `222852` | -- | the maintainer override; hold dir removed |
...
| 2 | Mainnet treasury | **Separate custody event** -- the maintainer word + `settlement_core` scrutiny + legal/tax counsel; testnet trial period first |
| 3 | SLC-L4 first pass | **Donor-amount privacy**; ZK pin `223327` (`sui::groth16` + Circom threshold); zkLogin/Seal/Nautilus out of v1; **lap still the maintainer-gated** |
...
**Status:** study complete; build waits on the maintainer lap word.
...
the maintainer asked: prioritize **A** (SLC-L4 backend), **B** (Realidream frontend), or **C** (codebase on-touch)?

**Composer recommendation: A's backend** -- with immediate step = **Claude implementation-shape ruling + the maintainer lap word**, not circuit code yet.
...
| Snapshot lap 4 | the maintainer's hands |
...
2. **Lap-open brief** -- What belongs in the active-designing scope note and expanding-prompt before Cursor's first `kg` pass once the maintainer gives lap word?
