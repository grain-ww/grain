...
This is the real design fork the maintainer's ask opens, and it deserves three named answers rather than one assumed one.
...
- It does not seat gold/iron/lead/zinc, or any Hoon-derived variance vocabulary, into Glow's own rune table -- that stays gated on a real, named need arising (per Option A/C above), and on the maintainer's own word.
