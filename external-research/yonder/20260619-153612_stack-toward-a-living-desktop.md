...
A person opens a window on AMD x86_64 hardware. Inside that window, a Rishi shell prompt accepts commands. The person types Mantra commands -- `mantra add`, `mantra commit`, `mantra status` -- and the shell executes them against the living `~/acme-owner` repository, growing a weave of its history. The window is drawn by Brushstroke: immediate-mode, computed fresh from values each frame. The whole application runs inside a Pond enclosure -- isolated, bounded, owned. No external process manager, no borrowed runtime, nothing hidden beneath it that does not belong to the family.
...
```
[Goal] Pond GUI -> Brushstroke -> Rishi REPL -> Mantra commands -> ~/acme-owner
