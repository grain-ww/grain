...
**What it changes for the arc.** Equinox 3's leading question no longer blocks design. The compiler question is settled in the affirmative, so the remaining work on that equinox is clean-room design against the capability model rather than a toolchain investigation -- and the next honest gate is the license read, which is the maintainer's word.
