...
|---|---|---|---|
| **P0** | **Pool host seam** | A minimal Pool that hosts *one* agent over Caravan's supervision + capability table -- an agent is admitted, runs bounded, and its state is a fold; witnessed on one real agent (the closed Open Ask loop is the natural first tenant) | Caravan capability table green (it is); the maintainer's word to open Pool |
...
- No Pool code, no Realidream view, no capability host is built; this prioritizes and scopes, gated behind the maintainer's word to open P0.
