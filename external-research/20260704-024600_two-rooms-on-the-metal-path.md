...
`tools/slc2a_ring3_metal.rish` and `tools/slc2b_keyboard.rish` link `-lwayland-client -lxkbcommon`. They require a compositor, a seat, often GNOME on the maintainer's machine. Door 2's metal close split this on purpose:
...
- **Live five (the maintainer):** human confirmations the automation cannot replace
...
| **WOV / MUR (was MALA) monarch season** | external-research fit memos | the maintainer gate on hardware and ledger |
...
Witness **152** proves sign -> append -> fold -> verify on metal. Scope note still names golden receipt `.bron`, temp log dir, and the maintainer edge affirmation for Open Asks. **Lesson:** the checkable room can be honestly partial; the vision room (lovable treasurer trust) may still ask for more before the product lap is *shared*.

### 3. PROPOSED SEED in work-in-progress

`work-in-progress/20260703-202312_crossing_manifest_seed.rye` carries a harness green at parity **151**, yet the file header marks **PROPOSED SEED**. Parse proof is checkable; module home and manifest law remain vision until the maintainer seats them. **Lesson:** one witness binds one claim -- not the whole horizon the filename suggests.
...
- **TWO_ROOMS seated as context canon** -- the maintainer word; this memo does not seat it
