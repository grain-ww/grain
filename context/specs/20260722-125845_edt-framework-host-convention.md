...
**Room:** checkable -- a stamp convention for one host, and one_clock_witness reads the zone it names
...
- **Stamps come from the maintainer, verbatim.** This note names which clock the wall reads here after the UDT season, so agents do not keep writing UTC stamps once the hand has asked for Eastern again.
...
the maintainer set Ubuntu **Settings -> Date & Time** to **New York** at `20260722.130034` (confirmed: `date` prints `EDT -0400`; `/etc/localtime` -> `America/New_York`). Plain `date '+%Y%m%d.%H%M%S'` is enough for stamps on this host now. `TZ=America/New_York` remains a fine explicit override if a shell ever inherits a different zone.
