...
Mand ring-3 is the destroying ring. It waits on the maintainer's word (counsel decision 5). Until then: ring-2 decides without touching; no key is destroyed by this pin. When authorized, ring-3 destroys keys and never records content; fails closed; records the removal fact before acting; asserts hold on its own path.
