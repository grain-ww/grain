...
the maintainer's ask: instead of one kernel called "Arvo" (or a single Rye-parallel), have **four switchable kernel variants**, like choosing among dual/tri/quad-boot options -- named `Riyo`, `Reya`, `Trey`, and `Tera`. Collision research on each, as of this search:
