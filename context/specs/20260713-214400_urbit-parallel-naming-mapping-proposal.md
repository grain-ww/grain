...
| **Realidream** | 18 | Proposed as the **Landscape parallel** -- the unified viewer | This is the one the maintainer explicitly asked to merge -- Realidream + Brushstroke + Comlink + Kumara into one unified-DAG editor-browser, playing the role Landscape plays as Urbit's own desktop web UI over Gall agents |
| **Dexter / Skate** | 18 / 10 | Landscape-tile-parallel (small, young lanes) | Both are early; a rename here is cheap (lowest reference counts in this whole table) and could be the very first, lowest-risk migration PR if the maintainer wants a concrete first step |
| **Brushstroke** | 92 | Folds into the Realidream-parallel unified app per the maintainer's merge request, rather than getting its own separate vane-style name | Brushstroke is the compositor; in the merged app it becomes the rendering layer inside the Landscape-parallel, not a standalone module |
...
- **The actual new names.** This document maps *roles*, not final names -- "Comlink plays Ames's role" does not mean Comlink should literally become named `Ames`; the maintainer's word on whether renames adopt Urbit's literal vane names or a fused Rye-flavored variant (as asked for the language itself: "merging Rye and Hoon... keeping the Hoon rune tradition") is the next decision, module by module.
