...
4. **Foundations visions** -- filed (`foundations/`); stablecoin law and Mantrapod investor hardening **deferred** until the maintainer reopens them.
...
```sh
export HOME=/home/xy/acme-owner/.build-home
export ZIG_LOCAL_CACHE_DIR=/home/xy/acme-owner/.zig-cache
export TMPDIR=/home/xy/acme-owner/.zig-cache/tmp
...
**Garden vs arena lexicon:** pristine std uses `init.arena`; agent rules and `context/TAME_GUIDANCE.md` now match. `garden` remains the warm local binding and Tally's future concept; a short `context/` lexicon note awaits the maintainer's ruling.
...
When the maintainer returns to build:
