...
- **`context/README.md`** -- the stale `~/acme-owner` workspace path corrected to the Glow OS repository; a line added naming `GLOW_PROFILE.bron` as where the specific contributor lives; the reader addressed generally; `the maintainer` -> `the maintainer` on this touch.
- **`foundations/README.md`** -- `the maintainer` -> `the maintainer` on this touch; the "adaptable to any community" framing it already carried left intact.
- **`ORGANIZING.md`** -- a `GLOW_PROFILE.bron` entry added beside the existing `GLOW_HOST.bron` entry.

Dated artifacts are untouched, per `dated_guard` and accrete-never-break. Session logs and decision records keep naming the maintainer plainly, exactly as the `acme-employee-voice` rule's own exceptions already require -- anonymizing a decision record would make it useless.
