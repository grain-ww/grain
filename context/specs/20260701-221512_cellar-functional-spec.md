...
The **master signing key** -- `[redacted-fingerprint]` -- is the root of trust, and it stays cold. It never enters the sandbox, and it never signs a routine archive. Its whole work is to certify the sandbox key and to stand as the fixed point every signature traces back to.

The **sandbox signing key** -- `[redacted-fingerprint]` -- signs every record. Because its authority is delegated, it can be revoked and reissued cleanly if the sandbox is ever compromised: the root endures untouched, and every record signed before the revocation remains verifiable against it.
