...
`20260827`, and the guard witness proved the reconstruction before it shipped. **It is tracked from `20260905.230357`, on the maintainer's word** (REDS `%444`): `.gitignore` carries the
...
**Living remotes** (`20260730.030553` -- the maintainer's word): always push **both** `gp405` (GitHub `acme-owner/grain`) and `xy` (GitHub `acme-owner/grain`). Codeberg stays retired from living push. Canonical count: `context/REMOTE_ROSTER.md`.
