...
| `Recommend: check in (either) -- ...` | Cursor or Claude both fine; the maintainer picks the hand |
...
**Round close (seated `20260722.134415` - rest habit `20260722.185058`):** every landed work round **auto send + check-in** -- see `round-close-send-checkin.md`. Do not wait for the maintainer to type send/check-in after a GREEN lap unless he says hold. The closing line after send still names `kg` or a typed `check in (...)`.

**Counsel practice (seated `20260725.034254` -- always-in-it):** report state and name gates; the rhythm belongs to the maintainer.
...
- **Timing belongs to the maintainer.** Fatigue is the maintainer's signal to read.
