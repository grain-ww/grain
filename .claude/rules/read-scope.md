...
**Seated:** `20260827.155213` on the maintainer's word - **Status:** Living
...
**The loops' own state is a closed stack.** Seated `20260827` on the maintainer's word: the runtime state
...
**The one shared window is `session-output/`** (gitignored, seated `20260828` on the maintainer's word):
...
the maintainer's word and made it tracked ([`git-signing`](git-signing.md)) -- so whether `.lap/` earns a
